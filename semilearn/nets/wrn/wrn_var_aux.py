...
class WideResNetVarAux(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, **kwargs):
        ...
        self.auxiliary1 = nn.Sequential(
            ...
            SepConv(
                channel_in=channels[3],
                channel_out=channels[4],
            ),
            nn.AvgPool2d(4, 4)
        )
        ...
        self.fc1 = nn.Linear(channels[4], num_classes)
        self.fc2 = nn.Linear(channels[4], num_classes)
        self.fc3 = nn.Linear(channels[4], num_classes)
        self.fc4 = nn.Linear(channels[4], num_classes)
    ...
    def forward(self, x, only_fc=False, only_feat=False, **kwargs):
        ...
        out4_feature = self.auxiliary4(features[3]).view(-1, self.channels)
...
def wrn_var_37_2_aux(pretrained=False, pretrained_path=None, **kwargs):
    model = WideResNetVarAux(first_stride=2, depth=28, widen_factor=2, **kwargs)
    if pretrained:
