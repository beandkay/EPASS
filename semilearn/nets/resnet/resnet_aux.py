...
class ResNet50Aux(nn.Module):
    ...
    def __init__(
        ...
        self.auxiliary4 = nn.AvgPool2d(1, 1)
    ...
    def forward(self, x, only_fc=False, only_feat=False, **kwargs):
        ...
        # out = self.fc(x)
        out1_feature = self.auxiliary1(features[0]).view(x.size(0), -1)
        out2_feature = self.auxiliary2(features[1]).view(x.size(0), -1)
        out3_feature = self.auxiliary3(features[2]).view(x.size(0), -1)
        out4_feature = self.auxiliary4(features[3]).view(x.size(0), -1)
...
def resnet50_aux(pretrained=False, pretrained_path=None, **kwargs):
    model = ResNet50Aux(**kwargs)
    return model