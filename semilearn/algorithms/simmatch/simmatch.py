...
class SimMatch_Net(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if self.epass:
            feat_proj = self.l2norm(
                (self.mlp_proj(feat) + self.mlp_proj_2(feat) +
                 self.mlp_proj_3(feat)) / 3)
        else:
...
class SimMatch(AlgorithmBase):
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        ...
        if args.dataset in [
                'stl10', 'cifar10', 'cifar100', 'svhn', 'superks',
                'tissuemnist', 'eurosat', 'superbks', 'esc50', 'gtzan',
                'urbansound8k', 'aclImdb', 'ag_news', 'dbpedia'
        ]:
    ...
    def set_model(self):
        ...
        model = SimMatch_Net(model,
                             proj_size=self.args.proj_size,
                             epass=self.args.use_epass)
        return model

    def set_ema_model(self):
        ema_model = self.net_builder(num_classes=self.num_classes)
        ema_model = SimMatch_Net(ema_model,
                                 proj_size=self.args.proj_size,
                                 epass=self.args.use_epass)
    ...
    def update_bank(self, k, labels, index):
        ...
        if self.use_ema_teacher:
            ...
            self.mem_bank[:, index] = F.normalize(
                self.ema_bank * self.mem_bank[:, index] +
                (1 - self.ema_bank) * k.t().detach())
        self.labels_bank[index] = labels.detach()

    def train_step(self, idx_lb, x_lb, y_lb, x_ulb_w, x_ulb_s):
        num_lb = y_lb.shape[0]
        num_ulb = len(x_ulb_w['input_ids']) if isinstance(
            x_ulb_w, dict) else x_ulb_w.shape[0]
        ...
        with self.amp_cm():
            ...
            if self.use_cat:
                ...
                logits_x_lb, ema_feats_x_lb = outs_x_lb['logits'], outs_x_lb[
                    'feat']
                # logits_x_lb, ema_feats_x_lb = self.model(x_lb)

                outs_x_ulb_w = self.model(x_ulb_w)
                ema_logits_x_ulb_w, ema_feats_x_ulb_w = outs_x_ulb_w[
                    'logits'], outs_x_ulb_w['feat']
                # ema_logits_x_ulb_w, ema_feats_x_ulb_w = self.model(x_ulb_w)

                outs_x_ulb_s = self.model(x_ulb_s)
                logits_x_ulb_s, feats_x_ulb_s = outs_x_ulb_s[
                    'logits'], outs_x_ulb_s['feat']
            ...
            with torch.no_grad():
                ...
                ema_probs_x_ulb_w = self.call_hook(
                    "dist_align",
                    "DistAlignHook",
            ...
            with torch.no_grad():
                ...
                factor = ema_probs_x_ulb_w.gather(
                    1, self.labels_bank.expand([num_ulb, -1]))
                ...
                if self.smoothing_alpha < 1:
                    ...
                    aggregated_prob = aggregated_prob.scatter_add(
                        1, self.labels_bank.expand([bs, -1]),
                        teacher_prob_orig)
            ...
            in_loss = torch.sum(-teacher_prob.detach() *
