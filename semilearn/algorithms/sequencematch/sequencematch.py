...
class SequenceMatch(AlgorithmBase):
    ...
    def __init__(self, args, net_builder,tb_log=None, logger=None):
        ...
        # sequencematch specificed arguments
    ...
    def train_step(self, x_lb, y_lb, idx_ulb, x_ulb_w, x_ulb_m, x_ulb_s):
        ...
        with self.amp_cm():
            if self.use_cat:
                ...
                logits_x_ulb_w, logits_x_ulb_m, logits_x_ulb_s = logits[num_lb:].chunk(3)
            else:
                logits_x_lb = self.model(x_lb)
                logits_x_ulb_m = self.model(x_ulb_m)
        ...
        tb_dict['train/unsup_loss_mw'] = unsup_loss_mw.item()
        tb_dict['train/unsup_loss_sm'] = unsup_loss_sm.item()
        tb_dict['train/unsup_loss_sw'] = unsup_loss_sw.item()
